<a href="{{ route('vendor.message.show', $conversation->order_package_id) }}" class="message-cont {{ $key == 0 ? 'pt-0' : '' }}">
    <div class="buyer-name">
        @php
            $sender = $conversation->latestMessage->sender;
        @endphp
        <h4>{{ $sender->is(Auth::user()) ? $sender->vendor->vendor_name :$sender->name }}</h4>
